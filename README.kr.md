...
- **중복 키 감지** - 중복된 JSON 키를 하이라이트하고 저장 시 경고
...
jvim은 JSON 객체 내 중복 키를 감지하여 경고합니다.

- **시각적 하이라이트** - 중복 키가 있는 라인을 빨간색 배경으로 표시
- **로드 시 경고** - 중복 키가 있는 파일을 열면 경고 메시지 표시
- **저장 시 보호** - 중복 키가 있으면 `:w`와 `:wq`가 차단됨 (`:w!` 또는 `:wq!`로 강제 저장)
- **`:dupkeys` 명령** - 모든 중복 키와 라인 번호 표시
- **중첩 지원** - 중첩 객체 내 중복 키도 감지
