...
- **내장 JSON/문자열 편집** - JSON 문자열과 멀티라인 문자열 값을 별도 패널에서 편집
...
이스케이프된 개행(`\n`)을 1개 이상 포함한 JSON 문자열 값에서 `es`를 입력하면, 디코딩된 텍스트를 별도 패널에서 편집할 수 있습니다. `:w`로 저장하면 다시 JSON 문자열로 이스케이프되어 부모 문서에 반영됩니다.

count prefix로 더 많은 개행 수를 요구할 수 있습니다. 예를 들어 `3es`는 디코딩된 문자열에 개행 문자가 3개 이상 있을 때만 열립니다.
...
- **내장 문자열 diff** - `es`로 각 패널의 멀티라인 문자열 값을 비교
